...
@Component({
  ...
  template: `
    <div
      ...
      class="fixed bottom-0 left-[280px] right-0 z-30 bg-surface-elevated/95 backdrop-blur-md border-t border-border-base grid grid-cols-[1fr_auto_1fr] items-center gap-10 px-10 h-[81px] transition-colors duration-300"
    >
      ...
      <div class="hidden md:flex items-center gap-8">
        ...
        <div class="flex items-center gap-3 w-48">
          ...
          <div class="flex flex-col gap-0.5 min-w-0 w-full">
            <span class="text-xs text-text-muted truncate">Cliente seleccionado</span>
            <span class="text-sm font-semibold text-text-primary truncate">{{ clientName() || 'Ninguno' }}</span>
          </div>
        </div>

        <div class="w-px h-8 bg-border-base shrink-0"></div>

        <div class="flex flex-col gap-0.5 w-48 min-w-0">
          <span class="text-xs text-text-muted truncate">Vehículo seleccionado</span>
          <div class="flex items-center gap-1.5 min-w-0 w-full">
            <span class="text-sm font-semibold text-text-primary truncate">{{ vehicleName() || 'Ninguno' }}</span>
            @if (!vehicleName()) {
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-text-muted shrink-0"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
            }
          </div>
        </div>
        ...
        @if (showBasePrice()) {
          ...
          <div class="flex flex-col gap-0.5 w-28 min-w-0">
            <span class="text-xs text-text-muted truncate">Precio base</span>
            <span class="text-sm font-semibold text-text-primary truncate">{{ basePrice() || 'S/ 0' }}</span>
          </div>
        }
      </div>
